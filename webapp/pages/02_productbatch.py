...
if not db_utils.st.session_state.db_connected:
    ...
    if st.button("Load Product Batch Data"):
        try:
            with db_utils.st.session_state.db_pool.acquire() as connection:
                with connection.cursor() as cursor:
                    # This table has a REF to Product_t. A direct SELECT * will show REF objects.
                    # To flatten, you'd need a custom query or a view.
                    # For simplicity, we'll show direct select, which reveals the REF.
                    cursor.execute(
                        '''
                        SELECT 
